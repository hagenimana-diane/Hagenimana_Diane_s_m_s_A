...
public class List_of_orders extends JFrame {
...
public List_of_orders() {
	...
	my_mysql obj=new my_mysql();
	 ...
	 JLabel titles=new JLabel("LIST OF  ALL CUSTOMERS");
	 contentPane.add(titles);
	 titles.setBounds(10, 20, 300, 30);
	 
	 String[] column= {"OrderID","FirstName","product name","OrderDate","status"};
...
public List_of_orders(DefaultTableModel defaultTableModel) {
...
//	 ResultSet rs=st.executeQuery("select OrderID,FirstName,Name,OrderDate,status from orders,customer,product WHERE orders.CustomerID=customer.CustomerID and product.ProductID=orders.ProductID"); 
  ...
  ResultSet rs=st.executeQuery("select OrderID,FirstName,Name,OrderDate,orders.status from orders,customer,product WHERE orders.CustomerID=customer.CustomerID and product.ProductID=orders.ProductID"); 
// Looping to store result in returning array data // 
	int i=0;
	while(rs.next())  {
	 for(int j=0;j<5;j++)
